...
//------------------------Parameter----------------------
`define TV_IN_gain "../tv/cdatafile/c.add.autotvin_gain.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter gain_DEPTH = 1;
reg [31 : 0] gain_OPERATE_DEPTH = 0;
parameter gain_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter add_continue_addr = 0;
parameter add_auto_start_addr = 0;
parameter gain_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_gain [gain_DEPTH - 1 : 0];
reg gain_write_data_finish;
...
//write gain reg
reg [31 : 0] write_gain_count = 0;
reg write_gain_run_flag = 0;
reg write_one_gain_data_done = 0;
...
assign TRAN_CRTL_BUS_write_data_finish = 1 & gain_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        gain_write_data_finish <= 0;
        write_gain_run_flag <= 0; 
        write_gain_count = 0;
        count_operate_depth_by_bitwidth_and_depth (gain_c_bitwidth, gain_DEPTH, gain_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_gain_run_flag <= 1; 
            write_gain_count = 0;
        end
        if (write_one_gain_data_done === 1) begin
            write_gain_count = write_gain_count + 1;
            if (write_gain_count == gain_OPERATE_DEPTH) begin
                write_gain_run_flag <= 0; 
                gain_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_gain
    integer write_gain_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] gain_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = gain_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_gain_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write gain data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (gain_c_bitwidth < 32) begin
                        gain_data_tmp_reg = mem_gain[write_gain_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < gain_c_bitwidth) begin
                                gain_data_tmp_reg[j] = mem_gain[write_gain_count][i*32 + j];
                            end
                            else begin
                                gain_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (gain_data_in_addr + write_gain_count * four_byte_num * 4 + i * 4, gain_data_tmp_reg, write_gain_resp);
                end
                process_busy = 0;
                write_one_gain_data_done <= 1;
                @(posedge clk);
                write_one_gain_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_gain_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [gain_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (gain_c_bitwidth , factor);
  fp = $fopen(`TV_IN_gain ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_gain); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < gain_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
