...
entity skipprefetch_Nelem_CFG_s_axi is
generic (
...
end entity skipprefetch_Nelem_CFG_s_axi;
...
-- 0x1c : reserved
-- 0x20 : Data signal of vb
--        bit 31~0 - vb[31:0] (Read/Write)
-- 0x24 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of skipprefetch_Nelem_CFG_s_axi is
    ...
    constant ADDR_N_DATA_0  : INTEGER := 16#18#;
    constant ADDR_N_CTRL    : INTEGER := 16#1c#;
    constant ADDR_VB_DATA_0 : INTEGER := 16#20#;
    constant ADDR_VB_CTRL   : INTEGER := 16#24#;
    ...
    signal int_n               : UNSIGNED(31 downto 0);
    signal int_vb              : UNSIGNED(31 downto 0);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_VB_DATA_0 =>
                        rdata_data <= RESIZE(int_vb(31 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_VB_DATA_0) then
                    int_vb(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_vb(31 downto 0));
                end if;
            end if;
        end if;
    end process;
