...
//------------------------Parameter----------------------
`define TV_IN_a "../tv/cdatafile/c.fetch_readalloc.autotvin_a.dat"
`define TV_IN_log "../tv/cdatafile/c.fetch_readalloc.autotvin_log.dat"
`define TV_OUT_ap_return "../tv/rtldatafile/rtl.fetch_readalloc.autotvout_ap_return.dat"
...
reg [31 : 0] log_OPERATE_DEPTH = 1;
parameter log_c_bitwidth = 32;
parameter ap_return_DEPTH = 1;
reg [31 : 0] ap_return_OPERATE_DEPTH = 0;
parameter ap_return_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter fetch_readalloc_continue_addr = 0;
parameter fetch_readalloc_auto_start_addr = 0;
parameter a_data_in_addr = 24;
parameter log_data_in_addr = 32;
...
reg [log_c_bitwidth - 1 : 0] reg_log;
reg log_write_data_finish;
...
//write log reg
reg [31 : 0] write_log_count = 0;
reg write_log_run_flag = 0;
reg write_one_log_data_done = 0;
//read ap_return reg
...
assign TRAN_AXILiteS_write_data_finish = 1 & a_write_data_finish & log_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        log_write_data_finish <= 0;
        write_log_run_flag <= 0; 
        write_log_count = 0;
        count_operate_depth_by_bitwidth_and_depth (log_c_bitwidth, log_DEPTH, log_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_AXILiteS_start_in === 1) begin
            log_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_log_run_flag <= 1; 
            write_log_count = 0;
        end
        if (write_one_log_data_done === 1) begin
            write_log_count = write_log_count + 1;
            if (write_log_count == log_OPERATE_DEPTH) begin
                write_log_run_flag <= 0; 
                log_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_log
    integer write_log_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer c_bitwidth;
    integer i;
    integer j;
    reg [31 : 0] log_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = log_c_bitwidth;
    process_num = 2;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num) ;
    while (1) begin
        process_2_finish <= 0;
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_log_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write log data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (log_c_bitwidth < 32) begin
                        log_data_tmp_reg = reg_log;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < log_c_bitwidth) begin
                                log_data_tmp_reg[j] = reg_log[i*32 + j];
                            end
                            else begin
                                log_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (log_data_in_addr + write_log_count * four_byte_num * 4 + i * 4, log_data_tmp_reg, write_log_resp);
                end
                process_busy = 0;
                write_one_log_data_done <= 1;
                @(posedge clk);
                write_one_log_data_done <= 0;
            end   
            process_2_finish <= 1;
        end
        @(posedge clk);
