...
//------------------------Parameter----------------------
`define TV_IN_start "../tv/cdatafile/c.read_ocm.autotvin_start.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter start_DEPTH = 1;
reg [31 : 0] start_OPERATE_DEPTH = 0;
parameter start_c_bitwidth = 32;
parameter start_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_start [start_DEPTH - 1 : 0];
reg start_write_data_finish;
...
reg write_one_start_data_done = 0;
...
assign TRAN_CTRL_BUS_write_data_finish = 1 & start_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        start_write_data_finish <= 0;
        write_start_run_flag <= 0; 
        write_start_count = 0;
        count_operate_depth_by_bitwidth_and_depth (start_c_bitwidth, start_DEPTH, start_OPERATE_DEPTH);
    end
    else begin
        ...
        if (write_one_start_data_done === 1) begin
            write_start_count = write_start_count + 1;
            if (write_start_count == start_OPERATE_DEPTH) begin
                write_start_run_flag <= 0; 
                start_write_data_finish <= 1;
            end
        end
    end
end
...
initial begin : write_start
    ...
    integer i;
    integer j;
    reg [31 : 0] start_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = start_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_start_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write start data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (start_c_bitwidth < 32) begin
                        start_data_tmp_reg = mem_start[write_start_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < start_c_bitwidth) begin
                                start_data_tmp_reg[j] = mem_start[write_start_count][i*32 + j];
                            end
                            else begin
                                start_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (start_data_in_addr + write_start_count * four_byte_num * 4 + i * 4, start_data_tmp_reg, write_start_resp);
                end
                process_busy = 0;
                write_one_start_data_done <= 1;
                @(posedge clk);
                write_one_start_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_start_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [start_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (start_c_bitwidth , factor);
  fp = $fopen(`TV_IN_start ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_start); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < start_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
