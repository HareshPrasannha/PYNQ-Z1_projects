...
//------------------------Parameter----------------------
`define TV_IN_source "../tv/cdatafile/c.read_ocm.autotvin_source.dat"
`define TV_OUT_ap_return "../tv/rtldatafile/rtl.read_ocm.autotvout_ap_return.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter source_DEPTH = 1;
reg [31 : 0] source_OPERATE_DEPTH = 1;
parameter source_c_bitwidth = 32;
parameter ap_return_DEPTH = 1;
reg [31 : 0] ap_return_OPERATE_DEPTH = 0;
parameter ap_return_c_bitwidth = 32;
parameter START_ADDR = 0;
parameter read_ocm_continue_addr = 0;
parameter read_ocm_auto_start_addr = 0;
parameter source_data_in_addr = 24;
...
reg  BREADY_reg = 0;
reg [source_c_bitwidth - 1 : 0] reg_source;
reg source_write_data_finish;
...
//write source reg
reg [31 : 0] write_source_count = 0;
reg write_source_run_flag = 0;
reg write_one_source_data_done = 0;
//read ap_return reg
...
assign TRAN_AXILiteS_write_data_finish = 1 & source_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        source_write_data_finish <= 0;
        write_source_run_flag <= 0; 
        write_source_count = 0;
        count_operate_depth_by_bitwidth_and_depth (source_c_bitwidth, source_DEPTH, source_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_source_run_flag <= 1; 
            write_source_count = 0;
        end
        if (write_one_source_data_done === 1) begin
            write_source_count = write_source_count + 1;
            if (write_source_count == source_OPERATE_DEPTH) begin
                write_source_run_flag <= 0; 
                source_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_source
    integer write_source_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] source_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = source_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_source_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write source data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (source_c_bitwidth < 32) begin
                        source_data_tmp_reg = reg_source;
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < source_c_bitwidth) begin
                                source_data_tmp_reg[j] = reg_source[i*32 + j];
                            end
                            else begin
                                source_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (source_data_in_addr + write_source_count * four_byte_num * 4 + i * 4, source_data_tmp_reg, write_source_resp);
                end
                process_busy = 0;
                write_one_source_data_done <= 1;
                @(posedge clk);
                write_one_source_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
