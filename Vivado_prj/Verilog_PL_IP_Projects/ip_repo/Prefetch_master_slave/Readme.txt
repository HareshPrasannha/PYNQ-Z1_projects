...
8. This IP also contains the latency measurments for each read requests send to the slave
   ACP port
9. The latency measurements is done as:
	a. CLK pulse at which the ARVALID and ARADDRS signal for a particular data address goes out is captured and stored in regs array arvalid_time.
	b. CLK pulse at which the corresponding ARREADY is received from the slave is captured in the regs array arready_time.
	c. Latency is computed by finding the difference between the above two values
		Latency = arready_time[index] - arvalid_time[index]
